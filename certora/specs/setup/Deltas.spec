/// This spec can be used to retrieve and set currency deltas
...
function getCurrencyDelta(address PoolManager, Conversions.Currency currency, address user) returns int256 {
    assert PoolManager == poolManager(), "Must only probe transient storage of the pool manager";
    return ghostCurrencyDelta[user][Conv.fromCurrency(currency)];
}

// use this to query currency deltas from your spec
function getCurrencyDeltaExt(Conversions.Currency currency, address account) returns int256 {
    return ghostCurrencyDelta[account][Conv.fromCurrency(currency)];
}

function setCurrencyDelta(Conversions.Currency currency, address user, int128 delta) returns (int256,int256) {
